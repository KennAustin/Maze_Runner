    ...
    for row_idx, row in enumerate(map_data):
        for col_idx, cell in enumerate(row):
            if cell == 'w':
                x = col_idx * cell_size
                y = row_idx * cell_size
                wall = Wall(x, y, cell_size, cell_size, 'image/waLl_stage3.png')
                walls.append(wall)
...
def draw_black_overlay(screen, cell_size, player, max_distance):
    player_grid_x = player.x // cell_size
    player_grid_y = player.y // cell_size

    for y in range(0, HEIGHT, cell_size):
        for x in range(0, WIDTH, cell_size):
            grid_rect = pygame.Rect(x, y, cell_size, cell_size)

            distance_x = abs(player_grid_x - x // cell_size)
            distance_y = abs(player_grid_y - y // cell_size)

            if distance_x > max_distance or distance_y > max_distance:
                pygame.draw.rect(screen, BLACK, grid_rect)

def main_3():
    pygame.mixer.music.load("music/ANOES.mp3")
    ...
    background_image = pygame.image.load('image/background_stage3.png').convert()

    player_spritesheet_right = 'image/player_right_stage3.png'
    player_spritesheet_left = 'image/player_left_stage3.png'
        
    player = Player(0, 544, 32, 32, player_spritesheet_right, player_spritesheet_left)
    enemies = []

    enemies.append(Enemy(96, 272, 96, 496, 32, 32, 'image/enemy_right_stage3.png', player))
    enemies.append(Enemy(144, 544, 720, 544, 32, 32, 'image/enemy_left_stage3.png', player))
    enemies.append(Enemy(304, 160, 720, 160, 32, 32, 'image/enemy_right_stage3.png', player))

    goals = []
    goals.append(Goal(288, 288, 'image/goal_stage3.png'))
    goals.append(Goal(352, 320, 'image/goal_stage3.png'))
    goals.append(Goal(416, 288, 'image/goal_stage3.png'))

    items = []
    items.append(Item(32, 480, 'image/item_stage3.png'))
    items.append(Item(224, 288, 'image/item_stage3.png'))

    map_data = load_map('map/map3.txt')
    ...
    goal_full_image = pygame.image.load('image/goal_full_stage3.png').convert_alpha()
    goal_empty_image = pygame.image.load('image/goal_empty_stage3.png').convert_alpha()
    ...
    while running:
        ...
        draw_black_overlay(screen, 32, player, max_distance)
        ...
        if info:
            ...
            player_info = pygame.image.load('image/player_info_stage3.png').convert_alpha()
            enemy_info = pygame.image.load('image/enemy_info_stage3.png').convert_alpha()
            heart_info = pygame.image.load('image/heart_full.png').convert_alpha()
            goal_info = pygame.image.load('image/goal_full_stage3.png').convert_alpha()
            ...
            font = pygame.font.Font("font/font.ttf", 50)
            stage1_text = font.render("STAGE 3", True, "#D8D9DA")
            stage1_rect = stage1_text.get_rect(center=(screen.get_width() // 2, 100))
            screen.blit(stage1_text, stage1_rect)

            info_text = [
                " = KAMU ADALAH SEORANG PENAMBANG YANG HARUS MELEWATI LABIRIN!",
                " = HINDARI MANUSIA GOA YANG ADA DI LABIRIN!",
                " = KAMU MEMILIKI TIGA NYAWA!",
                " = CARI KETIGA HARTA KARUN UNTUK MEMENANGKAN PERMAINAN",
            ]
