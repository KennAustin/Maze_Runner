...
import math
import random
from timer import start_timer, stop_timer, get_elapsed_time
...
class Player(Object):
    def __init__(self, x, y, width, height, spritesheet_right, spritesheet_left):
    ...
    def check_collision(self, new_x, new_y, walls):
        player_rect = pygame.Rect(new_x, new_y, self._width, self._height)

        for wall in walls:
            wall_rect = pygame.Rect(wall.x, wall.y, wall._width, wall._height)
    ...
    def check_collision_with_enemy(self, enemy):
        player_rect = pygame.Rect(self.x, self.y, self._width, self._height)
        enemy_rect = pygame.Rect(enemy.x, enemy.y, enemy._width, enemy._height)
    ...
    def check_collision_with_goal(self, goal):
        player_rect = pygame.Rect(self.x, self.y, self._width, self._height)
        goal_rect = pygame.Rect(goal.x, goal.y, goal._width, goal._height)
    ...
    def check_collision_with_item(self, item):
        player_rect = pygame.Rect(self.x, self.y, self._width, self._height)
        item_rect = pygame.Rect(item.x, item.y, item._width, item._height)
...
class Enemy(Object):
    def __init__(self, start_x, start_y, end_x, end_y, width, height, sprite_image, player):
        ...
        # Arah pergerakan (dari start ke end)
        self.dx = end_x - start_x
        self.dy = end_y - start_y
        self.normalize_direction()

        # Penanda untuk arah pergerakan
        self.moving_forward = True  # Mulai dengan bergerak maju

    def normalize_direction(self):
        # Normalisasi vektor arah agar panjangnya 1 (unit vector)
    ...
    def update(self, paused=False, info=False):
        if not paused and not info:  # Hanya update posisi musuh jika game tidak di-pause
            if self.moving_forward:
                # Pergerakan maju ke arah end_x, end_y
                new_x = self.x + self.speed * self.dx
                new_y = self.y + self.speed * self.dy

                # Cek apakah musuh sudah mencapai titik akhir
                if self.at_destination():
                    # Tandai bergerak mundur
                    self.moving_forward = False
            else:
                # Pergerakan mundur ke arah start_x, start_y
                new_x = self.x - self.speed * self.dx
                new_y = self.y - self.speed * self.dy

                # Cek apakah musuh sudah kembali ke titik awal
                if self.at_start():
                    # Tandai bergerak maju
    ...
    def update_animation(self, paused=False, info=False):  # Tambahkan parameter paused
        if not paused and not info:  # Hanya lakukan update animasi jika tidak di-pause
            self.frame_index = (self.frame_index + 1) % 4  # Ubah frame animasi

    def at_destination(self):
        # Periksa apakah musuh sudah mencapai titik akhir
    ...
    def at_start(self):
        # Periksa apakah musuh sudah kembali ke titik awal
    ...
    def check_collision(self, new_x, new_y, walls):
        ...
        new_rect = pygame.Rect(new_x, new_y, self._width, self._height)
        for wall in walls:
            wall_rect = pygame.Rect(wall.x, wall.y, wall._width, wall._height)
    ...
    def check_collision_with_player(self):
        ...
        player_rect = pygame.Rect(self.player.x, self.player.y, self.player._width, self.player._height)
        enemy_rect = pygame.Rect(self.x, self.y, self._width, self._height)
        return player_rect.colliderect(enemy_rect)

    def draw(self, screen):
        # Gambar musuh dengan menggunakan sprite_image
...
class PauseButton:
    ...
    def toggle_pause(self):
        ...
        # Mengubah gambar tombol sesuai status pause
        if self.paused:
...
class InfoButton:
    ...
    def check_for_input(self, position):
        if self.rect.collidepoint(position):
            self.toggle_info()  # Panggil metode toggle_info saat tombol ditekan

    def toggle_info(self):
        self.info = not self.info
        # Mengubah gambar tombol sesuai status info
        if self.info:
            self.image = pygame.transform.scale(self.image_orig, (self.rect.width // 2, self.rect.height // 2))
        else:
            self.image = self.image_orig
...
class Goal(Object):
    def __init__(self, x, y, image_path):
        super().__init__(x, y, 32, 32)  # Misalnya, kita asumsikan gambar berukuran 32x32
        self.image = pygame.image.load(image_path).convert_alpha()
        self.frame_index = 0
        self.frame_count = 0
        self.frame_delay = 3
    ...
    def update_animation(self, paused=False, info=False):  # Tambahkan parameter paused
        if not paused and not info:  # Hanya lakukan update animasi jika tidak di-pause
            ...
            if self.frame_count >= self.frame_delay:
                self.frame_index = (self.frame_index + 1) % 2
                self.frame_count = 0  # Reset frame_count setelah pergantian frame
    ...
    for row_idx, row in enumerate(map_data):
        for col_idx, cell in enumerate(row):
            if cell == 'w':
                x = col_idx * cell_size
                y = row_idx * cell_size
                wall = Wall(x, y, cell_size, cell_size, 'image/wall_Stage2.png')
                walls.append(wall)
    return walls
...
def main_2():
    pygame.mixer.music.load("music/TSFH.mp3")
    ...
    background_image = pygame.image.load('image/background_stage2.png').convert()

    player_spritesheet_right = 'image/player_right_stage2.png'
    player_spritesheet_left = 'image/player_left_stage2.png'

    player = Player(32, 32, 32, 32, player_spritesheet_right, player_spritesheet_left)

    enemies = []

    enemies.append(Enemy(96, 32, 96, 512, 32, 32, 'image/enemy_right_stage2.png', player))
    enemies.append(Enemy(232, 160, 368, 160, 32, 32, 'image/enemy_right_stage2.png', player))
    enemies.append(Enemy(232, 448, 432, 448, 32, 32, 'image/enemy_right_stage2.png', player))
    enemies.append(Enemy(544, 32, 544, 304, 32, 32,'image/enemy_left_stage2.png', player))
    enemies.append(Enemy(736, 480, 736, 288, 32, 32,'image/enemy_left_stage2.png', player))
    enemies.append(Enemy(672, 32, 672, 198, 32, 32,'image/enemy_left_stage2.png', player))

    goals = []
    goals.append(Goal(608, 320, 'image/goal_stage2.png'))
    goals.append(Goal(736, 32, 'image/goal_stage2.png'))
    goals.append(Goal(32, 544, 'image/goal_stage2.png'))

    items = []
    items.append(Item(32, 488, 'image/item_stage2.png'))

    map_data = load_map('map/map2.txt')
    ...
    goal_full_image = pygame.image.load('image/goal_full_stage2.png').convert_alpha()
    goal_empty_image = pygame.image.load('image/goal_empty_stage2.png').convert_alpha()
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if pause_button.rect.collidepoint(event.pos):
                    paused = not paused
                    stop_timer()
                    elapsed_pause_time = pygame.time.get_ticks() - start_time
                elif info_button.rect.collidepoint(event.pos):
                    info = not info
                    stop_timer()
                    elapsed_pause_time = pygame.time.get_ticks() - start_time
                else:
                    start_time = pygame.time.get_ticks() - elapsed_pause_time
                    start_timer()
            elif event.type == pygame.KEYDOWN:
        ...
        for item in items:
            item.draw(screen)
            item.update_animation(paused, info)

            if player.check_collision_with_item(item):
                player.speed = 16
                items.remove(item)
        ...
        if info:
            ...
            player_info = pygame.image.load('image/player_info_stage2.png').convert_alpha()
            enemy_info = pygame.image.load('image/enemy_info_stage2.png').convert_alpha()
            heart_info = pygame.image.load('image/heart_full.png').convert_alpha()
            goal_info = pygame.image.load('image/goal_full_stage2.png').convert_alpha()
            ...
            font = pygame.font.Font("font/font.ttf", 50)
            stage1_text = font.render("STAGE 2", True, "#D8D9DA")
            stage1_rect = stage1_text.get_rect(center=(screen.get_width() // 2, 100))
            screen.blit(stage1_text, stage1_rect)

            info_text = [
                " = KAMU ADALAH SEORANG PRIA YANG HARUS MELEWATI LABIRIN!",
                " = HINDARI MONSTER YANG ADA DI LABIRIN!",
                " = KAMU MEMILIKI TIGA NYAWA!",
                " = CARI KETIGA SABIT UNTUK MEMENANGKAN PERMAINAN",
            ]
        ...
        if not info and not paused:
            ...
            pause_text = font.render(f"Time: {remaining_time} seconds", True, WHITE)
            screen.blit(pause_text, (250, 10))
